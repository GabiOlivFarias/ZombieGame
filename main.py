...
zombie.y = 435
zombie.images = [
    'meninozombie1', 'meninozombie2', 'meninozombie3', 'meninozombie4',
    'meninozombie5', 'meninozombie6', 'meninozombie7', 'meninozombie8',
    'meninozombie9', 'meninozombie10'
]
zombie.fps = 40
...
def adjust_obstacle_speed(base_speed):
...
def start_game():
    global game_state, score, final_score, game_over, end_game, win_game, obstacles, obstacles_timeout
    game_state = "playing"
    pygame.mixer.music.play(-1)
    score = 0
    final_score = 0
...
def update():
    global velocity, isJumping, score, obstacles_timeout, game_over, end_game, final_score, win_game, game_state
    ...
    # Detecção de colisão com o fantasma
    if isJumping and zombie.colliderect(ghost):
        sounds.collect.play()
        score += 5
    ...
    if obstacles_timeout > adjust_obstacle_generation_interval():
        if len(obstacles) == 0 or (860 - obstacles[-1].x) > obstacles_gap:
    ...
    for tombstone in obstacles[:]:
        tombstone.x -= adjust_obstacle_speed(4)
        if tombstone.x < -50:
...
def draw():
    if game_state == "menu":
        ...
        screen.draw.text(
            "- Colete fantasmas para ganhar pontos.",
            center=(400, 250),
            color=yellow,
            fontsize=30,
        )
        screen.draw.text(
            "- Evite as lápides para não perder o jogo.",
            center=(400, 300),
            color=yellow,
            fontsize=30,
        )
